...
cp /home/Documents/ManualEdits/${subject}/talairach.xfm ${subject}/mri/transforms/talairach.xfm

recon-all -s ${subject} -nuintensitycor -normalization
cp ${subject}/mri/T1.mgz ${subject}/mri/brainmask.mgz # brain is already skullstripped, so just copy T1.mgz to brainmask.mgz

# # # # # use corrected talairach.xfm as initialization for the subcortical registration (talairach.lta)
